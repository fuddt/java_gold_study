...
**ただし注意！** `forEachOrdered()`を使うと並列処理のメリットが減る。順序を保証するためにスレッド間の同期が必要になるからね。
...
`collect(Collectors.toList())`の結果リストは元の順序を保持する。並列処理中の処理順序は不定だけど、結果のコレクションは順序が保たれるんだよね。
...
これが一番重要なポイントじゃね？特に**combinerが実際に使われる**ってとこ。
