...
Java Gold試験で頻出のStream中間操作をマスターするための資料だよ。
...
中間操作は**Stream**を返す操作のことだよ。終端操作が呼ばれるまで実際には実行されない（**遅延評価**）ってのが超重要。
...
**要素を1対多で変換してから平坦化**する操作。ネストしたStreamを1つのStreamにまとめるんだよね。
...
**要素をソート**する操作。自然順序またはComparatorでソートできるよ。
...
これで中間操作はバッチリだよね！実際にコードを動かして試してみることをお勧めするよ。
